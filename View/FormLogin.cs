...
namespace SistemaVentaAkira.View
{
    public partial class FormLogin : Form
    {
        ...
        {
           Validaciones.ValidarEmail(errorLogin, TxtUser, ref email);
        }
            ...
            if (TxtUser.Text == string.Empty || TextPassword.Text == string.Empty){
                ...
                if (controlador_usuario.verificarLoggin(TxtUser.Text, TextPassword.Text)){ //Se evalúa si el usuario se encuentra registrado
                    ...
                    else if (usuario[1].ToString() == "SUPERVISOR")
                    {
                        ...
                        MP_Supervisor.lblemailtxt.Text = usuario[4].ToString();
                ...
                if (TxtUser.Text == "ADMIN" && TextPassword.Text == "123"){
                Principal MP_Adm = new Principal();
                MP_Adm.lblroltxt.Text = "Administrador";
                MP_Adm.lblnombretxt.Text = "ADMIN";
                MP_Adm.lblemailtxt.Text = "admin@mail.com";
                this.Hide();
                MP_Adm.Show();
                MP_Adm.FormClosed += new FormClosedEventHandler(cerrarSesion);
            }
            else if (TxtUser.Text == "EMPLEADO" && TextPassword.Text == "123"){
                Menu_Empleado MP_Empleado = new Menu_Empleado();
                MP_Empleado.lblroltxt.Text = "Empleado";
                MP_Empleado.lblnombretxt.Text = "EMPLEADO";
                MP_Empleado.lblemailtxt.Text = "empleado@mail.com";
                this.Hide();
                MP_Empleado.Show();
                MP_Empleado.FormClosed += new FormClosedEventHandler(cerrarSesion);
            }
            else if (TxtUser.Text == "SUPERVISOR" && TextPassword.Text == "123"){
                Menu_Supervisor MP_Supervisor = new Menu_Supervisor();
                MP_Supervisor.lblroltxt.Text = "Supervisor";
                MP_Supervisor.lblnombretxt.Text = "SUPERVISOR";
                MP_Supervisor.lblemailtxt.Text = "supervisor@mail.com";
                this.Hide();
                MP_Supervisor.Show();
                MP_Supervisor.FormClosed += new FormClosedEventHandler(cerrarSesion);
            }
            else {
                MessageBox.Show("Usuario no encontrado. Intente de nuevo", "Acceso inválido", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }*/
        }
